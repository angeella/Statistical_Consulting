...
f <- as.formula(paste("confirmed_prop", "~", paste(c(var_EC), collapse=" + "), 
                     "+", paste(c(var_FIX), collapse=" + "),
                      ...
                      "+", "offset(confirmed/pop)", 
                     "+", paste(c(var_LD),collapse=" + "),
                      "+ (1|date2) + (1|Clusters)"))

model <- glmer(f, data = dat_shape,family = binomial(link = "logit"))
summary(model)
rsq(model,adj=TRUE)
#0.9725014
plot(model)
...
f <- as.formula(paste("confirmed_prop", "~", paste(c(var_EC[c(1:3)]), collapse=" + "), 
                    # "+", paste(c(var_FIX), collapse=" + "),
                   #   "+", paste(c(var_HS), collapse=" + "),
                   #   "+", "Clusters",
                     "+", paste(c(var_LD), collapse=" + "),
                      "+  offset(confirmed/pop) + Clusters1"))
