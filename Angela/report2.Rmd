...
dat_shape <- 
  dat_shape %>% arrange(id, date) %>%
  group_by(id) %>%
  mutate(confirmed_lag = dplyr::lag(confirmed, n = 14, default = NA))
dat_shape <- 
  dat_shape %>% 
  group_by(id) %>%
  mutate(confirmed_cum = cumsum(confirmed_lag,na.rm = TRUE), pop_cum = cumsum(pop, na.rm = TRUE))
dat_shape <- 
  dat_shape %>% 
  group_by(id) %>%
  mutate(confirmed_prop = confirmed_cum/pop_cum)
```
...
```{r}
ICCest(date2, confirmed_prop, dat_shape) #0.2833282
ICCest(id, confirmed_prop, dat_shape) #0.09110691
ICCest(Clusters, confirmed_prop, dat_shape) #0.03790068
```
...
```{r}
f <- as.formula(paste("confirmed_prop", "~", paste(c(var_EC), collapse=" + "), 
...
due <- c("KOR" ,"SGP")
tre <- c( "DEU" ,"SWE")
dat_shape <- dat_shape %>% mutate(id_f = as.factor(id))
...
dat_shapeScarpa1$testing_policyF <- recode_factor(dat_shapeScarpa1$testing_policyF,
                                                  ...
                                                  "2" = "C",
                                                  "3" = "D")
model <- glm(f, data = dat_shapeScarpa1,family = quasibinomial(link = "logit"))
summary(model)
MuMIn::r.squaredGLMM(model)
#           R2m       R2c
#[1,] 0.2796572 0.7935903
