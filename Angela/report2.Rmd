...
```{r}
a<-dat_shape %>% ggplot(aes(x=date2, y=confirmed_prop, group=Clusters, color=Clusters)) + geom_smooth()
...
dat_shape$EPOCH <- ifelse(dat_shape$date2 <= 0, "First", "Second")
dat_shape$EPOCH <- as.factor(dat_shape$EPOCH)
dat_shape$EPOCH <- ifelse(dat_shape$date <= "2020-03-01", "First", "Second")
dat_shape$EPOCH <- as.factor(dat_shape$EPOCH)
f <- as.formula(paste("confirmed_lag", "~", 
                      paste(c(var_EC[1]), collapse=" + "), 
                     "+", paste(c(var_FIX[c(1,3,4,5,6,7)]), collapse=" + "),
...
f <- as.formula(paste("log(confirmed_lag + 1)", "~", 
                    ...
                    #  "+", paste(c(var_HS), collapse=" + "),
                     "+", paste(c(var_LD[c(4:9)]),collapse=" + "),
                      "+ offset(log(pop)) + (1|id/Clusters) + (1|date2/EPOCH)"))
m1 <- pscl::zeroinfl(f, data = dat_shape, dist = "negbin")

f <- as.formula(paste("confirmed_lag", "~", 
                   #   paste(c(var_EC[c(4)]), collapse=" + "), 
                   #  "+", paste(c(var_FIX[c(1,3,4,5,6,7)]), collapse=" + "),
                   #   "+", paste(c(var_HS), collapse=" + "),
                     "+", paste(c(var_LD),collapse=" + "),
                      "+ offset(log(pop)) + Clusters"))
f5 = glmmTMB(f, family = "zig", random = ~ 1|EPOCH,data = dat_shape)
...
ggplot(data = dat_shape, aes(x=date, y=confirmed_count, group = Clusters)) + 
  geom_smooth()
...
model <- glmmTMB(f, data = dat_shape,family = poisson(link = "log"), 
                 na.action = na.omit)
summary(model)
check_overdispersion(model)
MuMIn::r.squaredGLMM(model,adj=TRUE)
plot_model(model)
pr<-ggpredict(model, "Clusters", type = "re")
pr<-ggeffect(model, "Clusters", type = "re")

pr1<-ggpredict(model, "id", type = "re")
